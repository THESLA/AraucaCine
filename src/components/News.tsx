...
const news = [
  {
    ...
    date: "28 Julio 2026",
    title: "AraucaCine acompañó el regreso de la hamburguesa en la Parrilla de Elvis — defienden su TRICAMPEONATO",
    excerpt: "AraucaCine estuvo presente en la Parrilla de Elvis, donde volvió la hamburguesa y esta vez defiende su tricampeonato. Probamos la #Reina: carne jugosa, mermelada de tocino dulcesita, queso apanado crujiente y queso crema parmesano con uchuba. Un sabor agridulce, una combinación tremenda — un mordisco y entendés por qué ganaron 3 veces seguidas.",
    shareText: "Volvió la hamburguesa y defiende su TRICAMPEONATO en la Parrilla de Elvis. AraucaCine estuvo allí. 🍔👑"
  },
  {
    id: "convite",
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-baseline gap-1 font-mono text-xs">
            ...
            <span className="text-muted-foreground">{String(news.length).padStart(2, "0")}</span>
          </div>
          <div className="flex gap-2">
            <button onClick={handlePrev}
              className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-border text-muted hover:text-foreground hover:border-accent/40 transition-all cursor-pointer bg-transparent">
              <ChevronLeft size={16} />
            </button>
        ...
        <div className="h-px bg-border relative overflow-hidden mb-6">
          <motion.div
            ...
            initial={{ width: "0%" }}
            animate={{ width: `${((activeIndex + 1) / news.length) * 100}%` }}
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            onClick={handleNext}
            className="bg-card border border-border rounded-xl overflow-hidden hover:border-accent/50 hover:shadow-lg hover:shadow-accent/10 transition-all duration-300 cursor-pointer"
          >
                ...
                <div className="relative">
                  ...
                  <div className="absolute bottom-full left-0 md:left-auto md:right-0 mb-2 z-20 bg-card border border-border rounded-xl shadow-xl p-4 min-w-[200px]">
                    ...
                    <div className="flex flex-col gap-2">
                      {/* WhatsApp */}
                      <a href={`https://api.whatsapp.com/send?text=${encode(`AraucaCine — ${news[activeIndex].title}\n${getNewsImageUrl(news[activeIndex].img)}\n\n${news[activeIndex].shareText}\n\n${SITE_URL}`)}`}
                      ...
                      {/* Facebook */}
                      <a href={`https://www.facebook.com/sharer/sharer.php?quote=${encode(`AraucaCine — ${news[activeIndex].title}`)}&u=${encode(SITE_URL)}`}
                        target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-2 text-xs text-foreground/80 hover:text-foreground bg-muted/50 hover:bg-muted rounded-lg px-3 py-2 transition-colors no-underline">
                      ...
                      {/* Twitter / X */}
                      <a href={`https://twitter.com/intent/tweet?text=${encode(`AraucaCine — ${news[activeIndex].title}`)}&url=${encode(SITE_URL)}`}
                        target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-2 text-xs text-foreground/80 hover:text-foreground bg-muted/50 hover:bg-muted rounded-lg px-3 py-2 transition-colors no-underline">
                      ...
                      {/* Copiar enlace */}
                      <button onClick={() => copyLink(news[activeIndex])}
          ...
          {news.map((_, i) => (
            <button key={i} onClick={() => setActiveIndex(i)}
              className={`w-2 h-2 rounded-full transition-all cursor-pointer border-none ${
                i === activeIndex ? "bg-accent w-4" : "bg-border hover:bg-muted-foreground/40"
              }`}
            />
          ))}
        </div>
